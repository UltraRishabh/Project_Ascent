...
@onready var red_side_area: CollisionShape3D = $RedCollider/RedSideArea/CollisionShape3D
@onready var blue_side_area: CollisionShape3D = $BlueCollider/BlueSideArea/CollisionShape3D
...
func _on_blue_side_area_body_entered(body: Node3D) -> void:
	if body.is_in_group("player"):
		player.velocity.y = 27.0

func _on_red_side_area_body_entered(body: Node3D) -> void:
	if body.is_in_group("player"):
